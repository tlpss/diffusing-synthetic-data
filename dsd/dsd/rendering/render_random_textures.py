...
from dsd.rendering.blender.polyhaven.polyhaven_materials import PolyhavenMaterials, add_texture_to_object
from dsd.rendering.blender.polyhaven.polyhaven_backgrounds import PolyhavenHDRIConfig, add_polyhaven_hdri_background_to_scene
from dsd.rendering.blender.renderer import render_scene, CyclesRendererConfig

def generate_random_texture_renders(
    source_directory, target_directory, num_renders_per_scene=1
):
    ...
    for image_dir in tqdm.tqdm(image_dirs):
        ...
        for i in range(num_renders_per_scene):
            ...
            # render the scene
            render_scene(CyclesRendererConfig(render_depth=False, render_segmentation=False, render_rgb=True), str(image_target_dir / f"render_{i}"))
            

if __name__ == "__main__":

    generate_random_texture_renders(
        source_directory= pathlib.Path("/home/tlips/Code/diffusing-synthetic-data/data/renders/shoes/2024-05-14_12-17-54"),
