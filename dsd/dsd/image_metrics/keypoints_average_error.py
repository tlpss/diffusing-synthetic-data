...
def calculate_average_error_for_dataset(
    ...
    for image, keypoints in tqdm(dataloader):
        ...
        for i in range(len(channel_config)):
            ...
            # print(f" channel {channel_config[i]}:Predicted: {kp} - GT: {gt_kp}")
    ...
    median_errors = [np.median(np.array(errors[i])).item() for i in range(len(channel_config))]
    for i in range(len(channel_config)):
        print(f"Average error for channel {channel_config[i]}: {average_errors[i]}")
        print("median error:", np.median(np.array(errors[i])))
        print("std deviation", np.std(np.array(errors[i])))

    print(f"Average error: {sum(average_errors)/len(average_errors)}")

    mae_dict = {}
    for i in range(len(channel_config)):
        if type(channel_config[i]) == list:
            channel_name = "" + "-".join(channel_config[i])
        else:
            channel_name = channel_config[i]
    ...
    for i in range(len(channel_config)):
        if type(channel_config[i]) == list:
            ...
            channel_name = channel_config[i]
        median_dict[channel_name] = median_errors[i]
    return mae_dict, median_dict
